...
import BlockChain
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'AES_Cryptography'))
import AES_Cryptography as AES_Cryptography
		...
		while True:
			try:
				...
				data = {
					...
					"msg" : str(AES_Cryptography.AESCipher(secret_key).encrypt(private_msg).decode('utf-8'))
				}
	...
	def __init__(self,address,port):
		...
		while True:
			...
			if (groupId == self.groupId):
				print(str(sender) + " : " + str(AES_Cryptography.AESCipher(self.key_aes).decrypt(msg).decode('utf-8')))
